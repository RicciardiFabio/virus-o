...
const MultiplayerArena: React.FC<MultiplayerArenaProps> = ({ playerName, roomId, audioEnabled, setAudioEnabled, onExit, onGameOver, sessionData }) => {
        ...
        const onState = (data: any) => {
            ...
            if (data.player && pid) {
                ...
                if (existing) {
                    ...
                    if (isHostRef.current) { existing.x = rxX; existing.y = rxY; }
                    
                    // L'infezione è permanente, non permettiamo al client di sovrascriverla con HEALTHY
                    if (existing.state === PlayerState.INFECTED) {
                        // Resta infetto
                    } else {
                        existing.state = data.player.state ?? existing.state;
                    }
                    
                    existing.activePowerUps = data.player.activePowerUps || {};
                    // DO NOT overwrite stunnedUntil and trappedUntil on the host from the client!
                    if (!isHostRef.current) {
                       existing.stunnedUntil = data.player.stunnedUntil ?? existing.stunnedUntil;
                       existing.trappedUntil = data.player.trappedUntil ?? existing.trappedUntil;
                    }
            ...
            if (data.type === 'v2_game_over') {
        if (onGameOver) onGameOver(data.winner, data.time);
        return;
      }

      if (data.isHostSync && !isHostRef.current) {
                if (Array.isArray(data.powerUps)) {
                    // Filtra i powerup già raccolti localmente per evitare che riappaiano
                    powerUpsRef.current = data.powerUps.filter((pu: any) => !collectedItemsRef.current.has(pu.id));
                }
                ...
                if (Array.isArray(data.players)) {
                  ...
                  data.players.forEach((p: any) => {
                    ...
                    seen.add(p.id);
                    if (p.id === myIdRef.current) {
                      if (myPlayerRef.current.state === PlayerState.HEALTHY && p.state === PlayerState.INFECTED) {
        ...
        const onPlayerJoined = (data: any) => {
             if (data.id && data.id !== myIdRef.current && !otherPlayersRef.current.has(data.id)) {
                 ...
                 otherPlayersRef.current.set(data.id, {
                     ...
                     x: s.x, y: s.y, targetX: s.x, targetY: s.y,
                     radius: PLAYER_SIZE, state: PlayerState.HEALTHY, isBot: false, speed: PLAYER_SPEED,
                     angle: 0, activePowerUps: {}, stunnedUntil: 0, trappedUntil: 0, trail: [],
                     lastUpdate: Date.now()
                 });
        ...
        const txInterval = setInterval(() => {
            ...
            const payload: any = {
                ...
                player: {
                    ...
                    // Rimosso stunnedUntil e trappedUntil per non sovrascrivere l'host
        ...
        const hostLoop = setInterval(() => {
            ...
            botsRef.current.forEach(bot => {
                ...
                if (now > ai.nextDecisionTime) {
                    ...
                    let tx = 0, ty = 0;
                    if (ai.isWandering) {
                        tx = Math.cos(ai.wanderAngle); ty = Math.sin(ai.wanderAngle);
                        ...
                        if (target) { tx = (target as Player).x - bot.x; ty = (target as Player).y - bot.y; }
                    ...
                    if (isNaN(bot.angle)) bot.angle = 0;
                }
                ...
                // Moltiplichiamo per 3 per compensare il tick rate di 50ms rispetto ai 16ms di requestAnimationFrame
                let speed = bot.speed * ai.speedMultiplier * speedFactor * (bot.state === PlayerState.INFECTED ? INFECTED_SPEED_BOOST : 1) * 3;
            ...
            const healthyPlayers = humans.filter(p => p.state === PlayerState.HEALTHY);
            if (healthyPlayers.length === 1 && humans.length > 1) {
                const winner = healthyPlayers[0];
                const time = Math.floor((Date.now() - startTime.current) / 1000);
                socket.emit('v2_state', { 
                    type: 'v2_game_over', 
                    roomId, 
                    winner: winner.name, 
                    time 
                });
                if (onGameOver) onGameOver(winner.name, time);
            }
    ...
    const draw = () => {
        ...
        const drawP = (p: RemotePlayer | Player, local: boolean) => {
            ...
            // Interpolate Remote (Host non interpola i bot perché li muove lui)
            ...
            const bodyColor = forceInfectedLook ? COLORS.INFECTED_BODY : (local ? COLORS.PLAYER_P1_BODY : COLORS.HEALTHY_BODY);
            const borderColor = frozen ? COLORS.FROZEN : (forceInfectedLook ? COLORS.INFECTED_BORDER : (local ? COLORS.PLAYER_P1_BORDER : COLORS.HEALTHY_BORDER));
            ...
            ctx.strokeStyle = borderColor; ctx.lineWidth = local ? 5 : 3; ctx.stroke();
        ...
        // L'host tratta i bot come locali per evitare che l'interpolazione li tiri indietro
        botsRef.current.forEach(p => drawP(p, isHostRef.current));
        ...
        if(mc) {
            ...
            if(mctx) {
                ...
                const drawDot = (p: any, c: string, meDot = false) => { 
                    if (!meDot && isPowerActive(p, PowerUpType.INVISIBLE, now)) return;
                    mctx.fillStyle=c; mctx.beginPath(); mctx.arc(p.x*s, p.y*s, 2, 0, Math.PI*2); mctx.fill(); 
                };
                ...
                drawDot(me, me.state===PlayerState.INFECTED ? '#ef4444' : '#22d3ee', true);
            }
        }
    };
    ...
    useEffect(() => {
        ...
        const loop = () => {
            ...
            const freezeActive = [myPlayerRef.current, ...Array.from(otherPlayersRef.current.values())]
                .some(v => v.state === PlayerState.HEALTHY && isPowerActive(v, PowerUpType.FREEZE, now));
            if (freezeActive && p.state === PlayerState.INFECTED) speedFactor *= 0.5;
            ...
            if (mag > 0.01) {
                ...
                // Trail update
                if (isPowerActive(p, PowerUpType.SPEED, now)) {
            ...
            for (let i = powerUpsRef.current.length - 1; i >= 0; i--) {
                ...
                if (checkCircleCollision(p, p.radius, pu, 35)) {
                    if (!collectedItemsRef.current.has(pu.id)) {
                        ...
                        // Tell Host
                        socket.emit('v2_state', { type: 'v2_collect_item', roomId: roomId, id: p.id, itemId: pu.id, itemType: pu.type });
                    }
                }
            }
  ...
  return (
    <div className="relative w-full h-full bg-slate-950 overflow-hidden touch-none select-none">
        ...
        {myPlayerRef.current.state === PlayerState.INFECTED && (
            <div className="absolute inset-0 z-40 flex items-center justify-center bg-rose-950/40 backdrop-blur-sm pointer-events-none">
                <div className="text-center animate-pulse">
                    <h2 className="text-6xl font-black text-rose-500 uppercase italic tracking-tighter">Signal Lost</h2>
                    <p className="text-rose-400 font-mono text-xs uppercase tracking-[0.5em] mt-2">Infection Detected - System Compromised</p>
                </div>
            </div>
        )}
